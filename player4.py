...
class Player(Tk.Frame):
    ...
    def __init__(self, parent, title=None, video=''):
        ...
        if _isMacOS:
            ...
            self.buttons_panel.bind(
                "<Button-1>", lambda event: setattr(self, "has_clicked_on_buttons_panel", event.y < 0))
            self.buttons_panel.bind(
                "<B1-Motion>", self._DetectButtonsPanelDragging)
            self.buttons_panel.bind(
                "<ButtonRelease-1>", lambda _: setattr(self, "has_clicked_on_buttons_panel", False))
    ...
    def _AnchorButtonsPanel(self):
        ...
        panel_y = self.parent.winfo_y() + video_height + 23
        panel_height = self.buttons_panel.winfo_height()
        panel_width = self.parent.winfo_width()
        self.buttons_panel.geometry(
            "%sx%s+%s+%s" % (panel_width, panel_height, panel_x, panel_y))
...
if __name__ == "__main__":
    ...
    while len(sys.argv) > 1:
        ...
        if arg.lower() in ('-v', '--version'):
            ...
            print(
                'usage: %s  [-v | --version]  [<video_file_name>]' % (sys.argv[0],))
            sys.exit(1)
